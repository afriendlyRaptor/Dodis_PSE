...
def build_kb(database, outputPath):
    ...
    # Bei Transformer-Modellen hidden_size aus HuggingFace AutoConfig,
    # bei klassischen Vektormodellen vocab.vectors_length.
    ...
    for qid, raw_json in cur.execute("SELECT id, data FROM entities"):
        ...
        if qid not in registered_ids:
            # Nullvektoren weil wir keine vorberechneten Entity-Embeddings haben.
            # Länge kommt aus entity_vector_length (FIX: nicht mehr hardgecodet).
            kb.add_entity(
                entity=qid, entity_vector=[0.0] * entity_vector_length, freq=3
            )
