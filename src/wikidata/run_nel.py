"""
NEL Pipeline: TEI-XML Dateien mit dem Wikidata-Modell verlinken und als HTML visualisieren.

Verlinkt Entities (<persName>, <orgName>, <placeName>) mit Wikidata Q-IDs
und generiert HTML-Visualisierungen.

Usage:
    python src/wikidata/run_nel.py <input>
    python src/wikidata/run_nel.py data/meine_texte/
    python src/wikidata/run_nel.py doc1.xml doc2.xml doc3.xml
    python src/wikidata/run_nel.py data/ --output results/
    python src/wikidata/run_nel.py data/ --model output/wikipedia/model-best
"""
...
DEFAULT_MODEL = "output/wikipedia/model-best"
...
def render_html(text, entities, doc_id=""):
    ...
    for ent in entities:
        ...
        ref = ent["ref"]
        ent_type = ent["type"]
        # Q-ID als Label anzeigen (z.B. "PER:Q76")
        label = f"{ent_type}:{ref}" if ref else ent_type
        css = f"ent {ent_type.lower()}" + ("" if ref else " missing")
        wikidata_url = f"https://www.wikidata.org/wiki/{ref}" if ref else None
        if wikidata_url:
            parts.append(
                f'<a href="{wikidata_url}" target="_blank" class="{css}" title="{ref}">'
    ...
    title = f"Wikidata NEL — {doc_id}" if doc_id else "Wikidata NEL Viewer"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="NEL Pipeline (Wikidata): TEI-XML verlinken und als HTML visualisieren"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        default="output/nel_results_wiki",
        help="Ausgabeverzeichnis fuer HTML-Dateien (default: output/nel_results_wiki)",
    )
    parser.add_argument(
    ...
    assert model_path.exists(), (
        ...
        "Zuerst run_wikidata_training.sh auf UBELIX ausführen."
    )
