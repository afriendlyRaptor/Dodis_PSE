...
def render_html(text, entities, show_ids=False, highlight_missing=False):
    ...
    for ent in entities:
        ...
        label = ent_type
        if show_ids and ref:
            # Wikidata Q-ID direkt als Label anzeigen
            label = f"{ent_type}:{ref}"
        ...
        # Wikidata-Link statt Dodis-Link
        wikidata_url = f"https://www.wikidata.org/wiki/{ref}" if ref else None

        if wikidata_url:
            span = f"""
            <a href="{wikidata_url}" target="_blank" class="{css_class}" title="{ref}">
                {ent_text}
...
<head>
<meta charset="UTF-8">
<title>Wikidata NEL Viewer</title>
<style>
body {{
...
def main():
    parser = argparse.ArgumentParser(
        description="TEI NEL Viewer (Wikidata) with clickable entities linking to wikidata.org"
    )
    parser.add_argument("input", help="Input TEI XML (bereits verlinkt mit Q-IDs)")
