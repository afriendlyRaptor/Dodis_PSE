...
und fügt ref-Attribute mit den verlinkten Wikidata-Q-IDs hinzu (z.B. ref="Q76").

Usage:
    python src/wikidata/link_tei.py input.xml output.xml
    python src/wikidata/link_tei.py input.xml output.xml --model output/wikipedia/model-best
"""
...
def link_paragraph(nlp, text, entity_refs):
    ...
    Gibt eine Zuordnung von XML-Element-ID zu kb_id (Q-ID) zurück.
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Entity Linking für TEI-XML Dokumente (Wikidata Q-IDs)"
    )
    parser.add_argument("input", help="Eingabe TEI-XML Datei")
    parser.add_argument(
        "output", help="Ausgabe TEI-XML Datei mit ref-Attributen (Q-IDs)"
    )
    parser.add_argument(
        "--model",
